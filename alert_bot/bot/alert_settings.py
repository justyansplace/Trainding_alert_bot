...
def render(user: User) -> tuple[str, InlineKeyboardMarkup]:
    ...
    if is_atr:
        ...
        threshold_line = f"<code>{current['pct']:.2f} %</code> от цены"
        scale = _bar(current["pct"], PCT_STEPS)
        explain = (
            "Процент проще соотнести с графиком, но не учитывает волатильность: "
            "0.25% для BTC — это близко, для валютной пары — очень далеко."
        )
